...
- PencilKit's pencil-only drawing policy does not stop its scroll view from panning with one finger. Require two touches on the canvas pan recognizer so a resting palm cannot move the page while Pencil drawing remains direct.
- Decode a sync payload once and pass the result along. Asking the same change three separate questions cost three full JSON decodes of every stroke sample on the main actor, and a deferred change paid again on every retry pass. Two watchdog kills came from that loop.
- Wall clock is the wrong measure for a watchdog kill. Persistence and decoding run on actors, so total time for a sync includes work the main thread never waited on. Measure the longest main-actor stall instead.
- An overlay added to a PKCanvasView is a sibling of the view PencilKit zooms, not a child of it, so it keeps its unzoomed size while the ink grows. Anchor each overlay to the content origin and scale it with `zoomScale`, or every hit test, lasso point, and drag distance is read at the wrong scale the moment the zoom leaves 1.
- A passing test for a feature is not proof the feature works. The lasso had a UI test that dragged a selection and asserted the stroke still existed afterwards, which held true whether or not the drag moved anything. Assert the thing the user would look at: where the ink ended up.
- Never call `zoom(to:animated:)` on a PKCanvasView that was never added to a window. It segfaults the whole test process, and XCTest blames whichever tests were running in parallel, which sends you looking for a bug in the app that is not there. Set `zoomScale` and `contentOffset` directly when the point is to check that programmatic movement still works.
- Debounce a sync outbox write after the first pending change, not instead of it. Persist immediately on the first enqueue so a failed store and a single library mutation stay durable. Coalesce follow-up strokes. Flush on checkpoint and synchronize. After joining an in-flight synchronize, push leftover pending work or a restore-then-metadata pair never leaves the device.
- A notebook with no snapshot on disk is dirty. Checkpointing only journals and deferred IDs skips the baseline write that undo recovery replays onto.
- Write a journal sidecar before the journal line. A crash then leaves an unused file. A line without its sidecar must be skipped, or recover replays a stroke with no ink.
- The iOS Simulator remaps `.complete` file protection to `.completeUntilFirstUserAuthentication`. Assert that a protected temp file is not `.none` or `.completeUnlessOpen` instead of requiring `.complete`.
